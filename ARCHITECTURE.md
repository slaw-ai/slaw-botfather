...
1a. **Skill registry — tower-mastered skills: DELIVERED (2026-06-07).** Botfather is the single source of truth for skills. They're authored/curated in the **Skill Registry** page (`skill_library` + singleton `skill_catalog_state`; draft → publish → deprecate; per-skill version monotonic, bumped only on published content-hash change). Skills propagate **tower → instance** — the reverse of the read-only `squad_skill` descriptor that flows up: an instance **pulls** the catalog (`GET /api/ingest/v1/skills`) and per-skill content (`/skills/:key`), then installs chosen skills onto **local, free-form squads** (squad composition stays local). A no-body `skills_updated` hint on the heartbeat/sync back-channel tells the instance the catalog advanced; it pulls on its own schedule and echoes `appliedSkillCatalogVersion`. Installed managed skills auto-refresh when the tower publishes a newer version. **Local skill authoring is locked when an instance is connected+enrolled** (standalone keeps it). SLAW source type `botfather` (read-only). Alert `skill_version_drift`. See `DESIGN-skill-registry.md`.
